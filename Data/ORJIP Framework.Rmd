...
for(i in 1:n)
{
  ...
  # HERE: Project positions into meter coordinates
}
...
usageag<-raster::aggregate((usage), fact=15)
usagespdf<-as(usageag, "SpatialPixelsDataFrame")
...
```{r Surveys, message=FALSE, warning=FALSE, cache=TRUE, error=FALSE,results='hide'}
############### TRANSECTS ##################################################



ESAS2016<-read.csv("ESAS 2016_MG.csv")

thin<-1
survey<-data.frame("Date"=ESAS2016$Date, "Lat"=ESAS2016$Latitude, "Lon"=ESAS2016$Longitude)[seq(1,nrow(ESAS2016),thin),]
...
covsTR<-data.frame("Date"=survey$Date[kp])
